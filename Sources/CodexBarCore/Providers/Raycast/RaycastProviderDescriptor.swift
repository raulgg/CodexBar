        ...
        ProviderDescriptor(
            ...
            fetchPlan: ProviderFetchPlan(
                ...
                pipeline: ProviderFetchPipeline(resolveStrategies: { context in
                    let callerTimeout = context.webTimeout.isFinite ? context.webTimeout : 30
                    return [ScriptFetchStrategy(
                        ...
                        sourceLabel: "web",
                        kind: .web,
                        timeout: max(30, callerTimeout),
                        resolveValues: { context in
                            guard context.settings?.raycast?.cookieSource != .off else { return nil }
                            let timeout = context.webTimeout.isFinite ? context.webTimeout : 15
                            let seconds = Int(min(30, max(1, timeout.rounded())))
                            return .init(settings: ["webTimeoutSeconds": String(seconds)])
                        },
                        isEnabled: { _ in true })]
                })),
